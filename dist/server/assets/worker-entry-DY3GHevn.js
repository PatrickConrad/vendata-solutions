...
async function getStartManifest() {
  const { tsrStartManifest } = await import("./_tanstack-start-manifest_v-IHLjF0HG.js");
...
function createStartHandler(cb) {
  ...
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import("./router-DkvrXX35.js").then((n) => n.r);
    }
    if (startEntry === null) {
