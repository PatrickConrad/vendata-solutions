...
async function getStartManifest() {
  const { tsrStartManifest } = await import("./_tanstack-start-manifest_v-CBSc7iYL.js");
...
function createStartHandler(cb) {
  ...
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import("./router-D1tVLFlb.js").then((n) => n.r);
    }
    if (startEntry === null) {
