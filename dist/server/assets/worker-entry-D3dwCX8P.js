...
async function getStartManifest() {
  const { tsrStartManifest } = await import("./_tanstack-start-manifest_v-B46s2d2o.js");
...
function createStartHandler(cb) {
  ...
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import("./router-BrVRcc1W.js").then((n) => n.r);
    }
    if (startEntry === null) {
